...
require(['jquery', 'F', 'store'], function ($, F, store) {
  ...
  $(function () {
    ...
    store.getEquMessage(options, function (res) {
      console.log(res)
      F.display('shiyongsm', res, function () {
        $('.shiyongsm').find('.content').append(res[0].instructions.Instructions)
      })
    })

    function initDom () {
      options.equid = F.getUrlParams('equid')
    }
